...
void find_largest_test()
{
    ...
    int nums2[] = {444, 11, 90, 600, 1, 22, 49, 12};
    int nums3[] = {0, 55, 200, 100, 32, 18, 99, 6};
    int nums4[] = {500, 1, 4, 17, 42, 18, 49, 12};
    
    // Test 1
    int res = find_largest(nums, sizeof(nums) / sizeof(int));
    IS_LARGEST(res, nums, sizeof(nums)/sizeof(int));
    // Test 2
    res = find_largest(nums2, sizeof(nums2) / sizeof(int));
    IS_LARGEST(res, nums2, sizeof(nums2)/sizeof(int));
    // Test 3
    res = find_largest(nums3, sizeof(nums3) / sizeof(int));
    IS_LARGEST(res, nums3, sizeof(nums3)/sizeof(int));
    // Test 4
    res = find_largest(nums4, sizeof(nums4) / sizeof(int));
    IS_LARGEST(res, nums4, sizeof(nums4)/sizeof(int));
    // for(int i =0; i < 5; ++i) {
...
void mem_tests()
{
    ...
    if (memcmp(buf1, buf2, BUFSZ) == 0) {
        printf("[*] %s (memset) | %d - PASS - Values are equal!\n", __FUNCTION__, __LINE__);
    }else {
        printf("[x] %s (memset) | %d - FAILED - Values are not equal!\n", __FUNCTION__, __LINE__);
    }
...
void string_tests()
{
    ...
    EQ((size_t)0, ex_strcmp(stringlist[0], stringlist2[0]));
    EQ((size_t)-1, ex_strcmp(stringlist[1], stringlist2[1]));
    EQ((size_t)0, ex_strcmp(stringlist[2], stringlist2[2]));
    EQ((size_t)1, ex_strcmp(stringlist[3], stringlist2[3]));
    EQ((size_t)0, ex_strcmp(stringlist[4], stringlist2[4]));
